...
if ( ! class_exists( 'Ava_Woo_Builder_Parser' ) ) {

	/**
	 * Define Ava_Woo_Builder_Parser class
	 */
	class Ava_Woo_Builder_Parser {

		public $processed_documents = array();
		private $ava_engine_object  = null;
		...
		public function get_template_content( $template_id ) {
			...
			$this->set_ava_engine_object();

			$render_method = apply_filters(
				'ava-woo-builder/get-template-content/render-method',
				ava_woo_builder_shop_settings()->get( 'widgets_render_method', 'macros' )
			);
			...
			$content = get_post_meta( $template_id, '_ava_woo_builder_content', true );
			...
			$this->reset_ava_engine_object();

			return $parsed;

		}

		public function set_ava_engine_object() {

			if ( ! function_exists( 'ava_engine' ) ) {
				return;
			}

			if ( null === $this->ava_engine_object ) {
				$this->ava_engine_object = ava_engine()->listings->data->get_current_object();
			}

			global $post;

			ava_engine()->listings->data->set_current_object( $post );

		}

		public function reset_ava_engine_object() {

			if ( ! function_exists( 'ava_engine' ) ) {
				return;
			}

			ava_engine()->listings->data->set_current_object( $this->ava_engine_object );
		}
		...
		public function replace_callback( $matches ) {
			...
			$args = apply_filters( 'ava-woo-builder/render-callback/custom-args', array() );
		...
		public function get_widget_from_macros( $macros ) {
			...
			if ( ! class_exists( $class_name ) ) {
				...
				$file =  glob( ava_woo_builder()->plugin_path( 'includes/widgets/' ) . 'archive-*/' . $macros . '.php' );
...
/**
 * Returns instance of Ava_Woo_Builder_Parser
 *
 * @return object
 */
function class_name() {
	return Ava_Woo_Builder_Parser::get_instance();
}
