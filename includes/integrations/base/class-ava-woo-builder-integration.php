...
if ( ! class_exists( 'Ava_Woo_Builder_Integration' ) ) {

	/**
	 * Define Ava_Woo_Builder_Integration class
	 */
	class Ava_Woo_Builder_Integration {
		...
		public function editor_styles() {
			...
			wp_enqueue_style(
				'ava-woo-builder-font',
				ava_woo_builder()->plugin_url( 'assets/css/lib/avawoobuilder-font/css/avawoobuilder.css' ),
				array(),
				ava_woo_builder()->get_version()
			);

			wp_enqueue_style(
				'ava-woo-builder-icons-font',
				ava_woo_builder()->plugin_url( 'assets/css/lib/ava-woo-builder-icons/ava-woo-builder-icons.css' ),
				array(),
				ava_woo_builder()->get_version()
			);

			wp_enqueue_style(
				'ava-woo-builder-editor',
				ava_woo_builder()->plugin_url( 'assets/css/editor.css' ),
				array(),
				ava_woo_builder()->get_version()
			);
		...
		public function include_wc_hooks() {
			...
			$rewrite = apply_filters( 'ava-woo-builder/integration/rewrite-frontend-hooks', false );
		...
		public function add_product_post_class( $classes ) {
			if (
				...
				if ( filter_var( ava_woo_builder_settings()->get( 'enable_product_thumb_effect' ), FILTER_VALIDATE_BOOLEAN ) ) {
					$classes[] = 'ava-woo-thumb-with-effect';
				}
			}
		...
		public function include_theme_integration_file() {
			...
			$int_file = ava_woo_builder()->plugin_path( "includes/integrations/themes/{$template}/functions.php" );
		...
		public function include_plugin_integration_file() {
			...
			foreach ( $plugins as $file => $condition ) {
				if ( true === call_user_func( $condition['cb'], $condition['args'] ) ) {
					require ava_woo_builder()->plugin_path( 'includes/integrations/plugins/' . $file );
				}
			}
		...
		public function open_canvas_wrap() {
			...
			if ( ! is_singular( ava_woo_builder_post_type()->slug() ) ) {
				return;
			}
		...
		public function in_elementor() {
			 ...
			 * @var bool $result
			 */
			return apply_filters( 'ava-woo-builder/in-elementor', $result );
		}
		...
		public function register_widgets( $widgets_manager ) {
			$global_available_widgets           = ava_woo_builder_settings()->get( 'global_available_widgets' );
			$single_available_widgets           = ava_woo_builder_settings()->get( 'single_product_available_widgets' );
			$archive_available_widgets          = ava_woo_builder_settings()->get( 'archive_product_available_widgets' );
			$archive_category_available_widgets = ava_woo_builder_settings()->get( 'archive_category_available_widgets' );
			$shop_available_widgets             = ava_woo_builder_settings()->get( 'shop_product_available_widgets' );

			require_once ava_woo_builder()->plugin_path( 'includes/base/class-ava-woo-builder-base.php' );
			...
			$doc_type = ava_woo_builder()->documents->get_current_type();

			if ( ! $doc_type ) {
				if ( get_post_type() === ava_woo_builder_post_type()->slug() ) {
					$doc_type = get_post_meta( get_the_ID(), '_elementor_template_type', true );
				}
			}

			$doc_type = apply_filters( 'ava-woo-builder/integration/doc-type', $doc_type );
		...
		public function is_setting_enabled( $type = 'custom_single_page' ) {
			...
			return filter_var( ava_woo_builder_shop_settings()->get( $type ), FILTER_VALIDATE_BOOLEAN );
		...
		public function register_category() {
			...
			$ava_woo_builder_cat = 'ava-woo-builder';

			$elements_manager->add_category(
				$ava_woo_builder_cat,
				array(
					'title' => esc_html__( 'Ava Woo Builder', 'ava-woo-builder' ),
					'icon'  => 'font',
				),
				1
			);
		...
		public function add_controls( $controls_manager ) {
			...
			$grouped = array(
				'ava-woo-box-style' => 'Ava_Woo_Group_Control_Box_Style',
			);
		...
		public function include_control( $class_name, $grouped = false ) {
			...
			if ( ! file_exists( ava_woo_builder()->plugin_path( $filename ) ) ) {
				return false;
			}

			require ava_woo_builder()->plugin_path( $filename );
...
/**
 * Returns instance of Ava_Woo_Builder_Integration
 *
 * @return object
 */
function ava_woo_builder_integration() {
	return Ava_Woo_Builder_Integration::get_instance();
}