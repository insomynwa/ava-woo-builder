...
if ( ! class_exists( 'Ava_Woo_Builder_Documents' ) ) {

	/**
	 * Define Ava_Woo_Builder_Documents class
	 */
	class Ava_Woo_Builder_Documents {
		...
		function __construct() {
			...
			if ( ! class_exists( 'Ava_Theme_Core' ) && ! class_exists( 'Ava_Engine' ) ) {
		...
		public function get_document_types() {
			...
			return array(
				'single'   => array(
					'slug'  => ava_woo_builder_post_type()->slug(),
					'name'  => __( 'Single', 'ava-woo-builder' ),
					'file'  => 'includes/documents/class-ava-woo-builder-document-single.php',
					'class' => 'Ava_Woo_Builder_Document',
				),
				'archive'  => array(
					'slug'  => ava_woo_builder_post_type()->slug() . '-archive',
					'name'  => __( 'Archive', 'ava-woo-builder' ),
					'file'  => 'includes/documents/class-ava-woo-builder-document-archive-product.php',
					'class' => 'Ava_Woo_Builder_Archive_Document_Product',
				),
				'category' => array(
					'slug'  => ava_woo_builder_post_type()->slug() . '-category',
					'name'  => __( 'Category', 'ava-woo-builder' ),
					'file'  => 'includes/documents/class-ava-woo-builder-document-archive-category.php',
					'class' => 'Ava_Woo_Builder_Archive_Document_Category',
				),
				'shop'     => array(
					'slug'  => ava_woo_builder_post_type()->slug() . '-shop',
					'name'  => __( 'Shop', 'ava-woo-builder' ),
					'file'  => 'includes/documents/class-ava-woo-builder-document-archive.php',
					'class' => 'Ava_Woo_Builder_Shop_Document',
				),
			);
		...
		/**
		 * Register apropriate document types for 'ava-woo-builder' post type
		 *
		...
		public function register_elementor_document_types( $documents_manager ) {
			...
			require ava_woo_builder()->plugin_path( 'includes/documents/class-ava-woo-builder-document-base.php' );

			$document_types = $this->get_document_types();

			require ava_woo_builder()->plugin_path( 'includes/documents/class-ava-woo-builder-not-supported.php' );
			$documents_manager->register_document_type( 'ava-woo-builder-not-supported', 'Ava_Woo_Builder_Document_Not_Supported' );

			foreach ( $document_types as $type => $data ) {
				require ava_woo_builder()->plugin_path( $data['file'] );
