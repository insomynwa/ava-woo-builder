<?php
/**
 * Ava Woo Builder post type template
 */
...
if ( ! class_exists( 'Ava_Woo_Builder_Post_Type' ) ) {

	/**
	 * Define Ava_Woo_Builder_Post_Type class
	 */
	class Ava_Woo_Builder_Post_Type {
		...
		protected $post_type = 'ava-woo-builder';
		protected $meta_key  = 'ava-woo-builder-item';
		...
		public function init() {
			...
			add_action( 'admin_action_ava_woo_new_template', array( $this, 'create_template' ) );
		...
		public function create_template() {
			...
			if ( ! current_user_can( 'edit_posts' ) ) {
				wp_die(
					esc_html__( 'You don\'t have permissions to do this', 'ava-woo-builder' ),
					esc_html__( 'Error', 'ava-woo-builder' )
				);
			}
			...
			if ( $template ) {
				if ( ! isset( $templates[ $template ] ) ) {
					wp_die(
						esc_html__( 'This template not registered', 'ava-woo-builder' ),
						esc_html__( 'Error', 'ava-woo-builder' )
					);
				}
			...
			if ( ! $template_id ) {
				wp_die(
					esc_html__( 'Can\'t create template. Please try again', 'ava-woo-builder' ),
					esc_html__( 'Error', 'ava-woo-builder' )
				);
			}
		...
		public function enqueue_templates_popup( $hook ) {
			...
			wp_enqueue_style(
				'ava-woo-builder-template-popup',
				ava_woo_builder()->plugin_url( 'assets/css/template-popup.css' )
			);

			wp_enqueue_script(
				'ava-woo-builder-template-popup',
				ava_woo_builder()->plugin_url( 'assets/js/template-popup.js' ),
				array( 'jquery' ),
				ava_woo_builder()->get_version(),
				true
			);

			wp_localize_script( 'ava-woo-builder-template-popup', 'AvaWooPopupSettings', array(
				'button' => '<a href="#" class="page-title-action ava-woo-new-template">' . __( 'Create from predesigned template', 'ava-woo-builder' ) . '</a>',
			) );
		...
		public function predesigned_single_templates() {
			...
			$base_url = ava_woo_builder()->plugin_url( 'includes/templates/single/' );
			$base_dir = ava_woo_builder()->plugin_path( 'includes/templates/single/' );

			return apply_filters( 'ava-woo-builder/predesigned-single-templates', array(
				'layout-1' => array(
		...
		public function predesigned_archive_templates() {
			...
			$base_url = ava_woo_builder()->plugin_url( 'includes/templates/archive/' );
			$base_dir = ava_woo_builder()->plugin_path( 'includes/templates/archive/' );

			return apply_filters( 'ava-woo-builder/predesigned-archive-templates', array(
				'layout-1' => array(
					'content' => $base_dir . 'layout-1/template.json',
					'thumb'   => $base_url . 'layout-1/thumbnail.png',
				),
		...
		public function predesigned_category_templates() {
			...
			$base_url = ava_woo_builder()->plugin_url( 'includes/templates/category/' );
			$base_dir = ava_woo_builder()->plugin_path( 'includes/templates/category/' );

			return apply_filters( 'ava-woo-builder/predesigned-category-templates', array(
				'layout-1' => array(
					'content' => $base_dir . 'layout-1/template.json',
					'thumb'   => $base_url . 'layout-1/thumbnail.png',
				),
		...
		public function predesigned_shop_templates() {
			...
			$base_url = ava_woo_builder()->plugin_url( 'includes/templates/shop/' );
			$base_dir = ava_woo_builder()->plugin_path( 'includes/templates/shop/' );

			return apply_filters( 'ava-woo-builder/predesigned-shop-templates', array(
				'layout-1' => array(
					'content' => $base_dir . 'layout-1/template.json',
					'thumb'   => $base_url . 'layout-1/thumbnail.png',
				),
		...
		public function template_popup() {
			...
			$action = add_query_arg(
				array(
					'action' => 'ava_woo_new_template',
				),
				esc_url( admin_url( 'admin.php' ) )
			);

			include ava_woo_builder()->get_template( 'template-popup.php' );

		}

		/**
		 * Maybe fix document types for Ava Woo templates
		 *
		 * @return void
		 */
		public function fix_documents_types() {

			if ( ! isset( $_GET['fix_ava_woo_templates'] ) ) {
				return;
			}
		...
		/**
		 * Ava Woo Builder page
		 */
		public function add_templates_page() {

			add_submenu_page(
				'woocommerce',
				esc_html__( 'Ava Woo Templates', 'ava-woo-builder' ),
				esc_html__( 'Ava Woo Templates', 'ava-woo-builder' ),
				'edit_pages',
		...
		/**
		 * Returns Ava Woo Builder meta key
		 *
		 * @return string
		 */
		...
		/**
		 * Add elementor support for ava woo builder items.
		 */
		...
		public function register_post_type() {
			...
			$labels = array(
				...
				'add_new'       => esc_html__( 'Add New Template', 'ava-woo-builder' ),
				'add_new_item'  => esc_html__( 'Add New Template', 'ava-woo-builder' ),
				'edit_item'     => esc_html__( 'Edit Template', 'ava-woo-builder' ),
				'menu_name'     => esc_html__( 'Ava Woo Templates', 'ava-woo-builder' ),
			);
		...
		public function init_meta() {
			...
			new Cherry_X_Post_Meta( array(
				...
				'title'         => esc_html__( 'Template Settings', 'ava-woo-builder' ),
				...
				'fields'        => array(
					'_sample_product' => array(
						...
						'label'             => esc_html__( 'Sample Product for Editing (if not selected - will be used latest added)', 'ava-woo-builder' ),
		...
		public function get_products() {
			...
			$default = array(
				'' => __( 'Select Product...', 'ava-woo-builder' ),
			);
		...
		public function get_templates_list_for_options( $type = 'all' ) {
			...
			$default = array(
				'' => esc_html__( 'Select Template...', 'ava-woo-builder' ),
			);
...
/**
 * Returns instance of Ava_Woo_Builder_Post_Type
 *
 * @return object
 */
function ava_woo_builder_post_type() {
	return Ava_Woo_Builder_Post_Type::get_instance();
}
