...
if ( ! class_exists( 'Ava_Woo_Builder_Template_Functions' ) ) {

	/**
	 * Define Ava_Woo_Builder_Template_Functions class
	 */
	class Ava_Woo_Builder_Template_Functions {
		...
		public function get_product_thumbnail( $image_size = 'thumbnail_size', $use_thumb_effect = false, $attr = array() ) {
			...
			return apply_filters( 'ava-woo-builder/template-functions/product-thumbnail', $html );
		}
		...
		public function add_thumb_effect( $html, $product, $image_size, $attr ) {
			$thumb_effect   = ava_woo_builder_settings()->get( 'product_thumb_effect' );
			...
			$html = sprintf(
				'<div class="ava-woo-product-thumbs effect-%3$s"><div class="ava-woo-product-thumbs__inner">%1$s%2$s</div></div>',
		...
		public function get_category_thumbnail( $category_id, $image_size = 'thumbnail_size' ) {
			...
			return apply_filters( 'ava-woo-builder/template-functions/category-thumbnail', $html );
		}
		...
		public function get_product_sku() {
			...
			if ( $product->get_sku() ) {
				...
				return apply_filters( 'ava-woo-builder/template-functions/sku', $sku );
		...
		public function get_product_rating() {
			...
			$format = apply_filters(
				'ava-woo-builder/template-functions/product-rating',
		...
		public function get_product_price() {
			...
			return apply_filters( 'ava-woo-builder/template-functions/product-price', $price_html );
		}
		...
		public function get_product_excerpt() {
			...
			return apply_filters( 'ava-woo-builder/template-functions/product-excerpt', get_the_excerpt( $product->get_id() ) );
		}
		...
		public function get_product_add_to_cart_button( $classes = array() ) {
			...
			if ( $product ) {
				$defaults = apply_filters(
					'ava-woo-builder/template-functions/product-add-to-cart-settings',
					array(
						'quantity'   => 1,
 ...
 * @return object
 */
function ava_woo_builder_template_functions() {
	return Ava_Woo_Builder_Template_Functions::get_instance();
}