...
/**
 * Define Ava_Dashboard_License_Manager class
 */
class License_Manager {
	 ...
	 * @var boolean
	 */
	public $license_data_key = 'ava-license-data';
	...
	public function __construct() {
		...
		$this->sys_messages = apply_filters( 'ava_dashboard_license_sys_messages', array(
			'internal'     => 'Internal error. Please, try again later',
			'server_error' => 'Server error. Please, try again later',
		) );

		add_action( 'wp_ajax_ava_license_action', array( $this, 'ava_license_action' ) );

		add_action( 'wp_ajax_ava_dashboard_debug_action', array( $this, 'ava_dashboard_debug_action' ) );
	 ...
	 * @return void
	 */
	public function ava_license_action() {
	...
	public function license_expire_check() {
		...
		$ava_dashboard_license_expire_check = get_site_transient( 'ava_dashboard_license_expire_check' );

		if ( $ava_dashboard_license_expire_check ) {
			return false;
		}

		Utils::license_data_expire_sync();

		set_site_transient( 'ava_dashboard_license_expire_check', 'true', HOUR_IN_SECONDS * 12 );
	}
	...
	public function maybe_theme_core_license_exist() {
		...
		$ava_theme_core_key = get_option( 'ava_theme_core_license', false );

		if ( ! $ava_theme_core_key ) {
			return false;
		}

		$ava_theme_core_license_sync = get_option( 'ava_theme_core_sync', 'false' );

		if ( filter_var( $ava_theme_core_license_sync, FILTER_VALIDATE_BOOLEAN ) ) {
			return false;
		}

		$license_list = Utils::get_license_data( 'license-list', [] );

		if ( array_key_exists( $ava_theme_core_key, $license_list ) ) {
			return false;
		}

		$responce = $this->license_action_query( 'activate_license', $ava_theme_core_key );

		$responce_data = isset( $responce['data'] ) ? $responce['data'] : [];

		$license_list[ $ava_theme_core_key ] = array(
			'licenseStatus'  => 'active',
			'licenseKey'     => $ava_theme_core_key,
			'licenseDetails' => $responce_data,
		);

		update_option( 'ava_theme_core_sync', 'true' );
	...
	public function maybe_modify_tm_license_data() {
		...
		$is_modify_tm_license_data = get_option( 'ava_is_modify_tm_license_data', 'false' );
		...
		if ( $license_list && ! empty( $license_list ) ) {
			...
			update_option( 'ava_is_modify_tm_license_data', 'true' );
		}
	}

	/**
	 * [ava_dashboard_debug_action description]
	 * @return [type] [description]
	 */
	 ...
	 * @return void
	 */
	public function ava_dashboard_debug_action() {
		...
		switch ( $license_action ) {
			...
			case 'license-expire-check':
				delete_site_transient( 'ava_dashboard_license_expire_check' );
			...
			case 'modify-tm-license-data':
				update_option( 'ava_is_modify_tm_license_data', 'false' );
