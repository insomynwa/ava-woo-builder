<?php
namespace Ava_Dashboard;

use Ava_Dashboard\Dashboard as Dashboard;
...
class Plugin_Manager {
	...
	/**
	 * [$ava_banners_url description]
	 * @var string
	 */
	public $ava_banners_url = 'https://account.blockcroco.com/free-download/images/avabanners/';

	/**
	 * [$ava_changelog_url description]
	 * @var string
	 */
	public $ava_changelog_url = 'https://blockcroco.com/wp-content/uploads/ava-changelog/%s.json';
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_ava_dashboard_plugin_action', array( $this, 'plugin_action' ) );
		...
		if ( ! empty( $registered_plugins ) ) {
			foreach ( $registered_plugins as $plugin_file => $plugin_data ) {
				...
				$this->registered_plugins_data[ $plugin_file ] = array(
					...
					'tested'     => '',
					'banners'    => array(
						'high' => sprintf( 'https://account.blockcroco.com/free-download/images/avabanners/%s.png', $plugin_data['slug'] ),
						'low'  => sprintf( 'https://account.blockcroco.com/free-download/images/avabanners/%s.png', $plugin_data['slug'] ),
					),
					'version'       => false,
					'changelog'     => false,
	...
	public function changelog_remote_query( $slug ) {
		...
		$response = wp_remote_get( sprintf( $this->ava_changelog_url, $slug ) );
	...
	public function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data ) {
		...
		if ( array_key_exists( $plugin_file, $this->registered_plugins_data ) && empty( $plugin_data['update'] ) ) {
			...
			$plugin_meta['view-details'] = sprintf( '<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
				...
				esc_attr( sprintf( __( 'More information about %s', 'ava-tricks' ), $this->registered_plugins_data[ $plugin_file ]['name'] ) ),
	...
	public function check_update( $data ) {
		...
		delete_site_transient( 'ava_dashboard_remote_ava_plugin_list' );
	...
	public function check_new_update_version( $plugin_data = false ) {
		...
		$remote_plugin_data = $this->get_remote_ava_plugin_list();
	 ...
	 * @return array|bool
	 */
	public function get_remote_ava_plugin_list() {

		$remote_ava_plugin_list = get_site_transient( 'ava_dashboard_remote_ava_plugin_list' );

		if ( $remote_ava_plugin_list ) {
			return $remote_ava_plugin_list;
		}
		...
		$remote_ava_plugin_list = $response['data'];

		set_site_transient( 'ava_dashboard_remote_ava_plugin_list', $remote_ava_plugin_list, HOUR_IN_SECONDS * 12 );
	...
	public function get_plugin_data_list() {
		...
		$ava_plugin_list    = $this->get_remote_ava_plugin_list();
		$user_plugin_list   = $this->get_user_plugins();
		$registered_plugins = Dashboard::get_instance()->get_registered_plugins();

		$plugins_list = [];

		if ( ! empty( $ava_plugin_list ) ) {
			foreach ( $ava_plugin_list as $key => $plugin_data ) {
	...
	public function allow_unsafe_urls ( $args ) {
		...
		if ( isset( $_REQUEST['action'] ) && 'ava_dashboard_plugin_action' === $_REQUEST['action'] ) {
