(function () {
	...
	Vue.component( 'license-page', {
		template: '#ava-dashboard-license-page',

		data: function() {
			return {
				allAvaPlugins: window.AvaDashboardPageConfig.allAvaPlugins || {},
				licenseList: window.AvaDashboardPageConfig.licenseList || [],
				...
				debugConsoleAction: '',
				debugActionList: window.AvaDashboardPageConfig.debugActions || [],
		...
		computed: {
			...
			installedPluginList: function() {
				...
				for ( let pluginSlug in this.allAvaPlugins ) {

					if ( this.allAvaPlugins[ pluginSlug ][ 'isInstalled' ] ) {

						let pluginData = this.allAvaPlugins[ pluginSlug ];
		...
		methods: {
			...
			updateUserPluginData: function( data ) {
				...
				this.allAvaPlugins[ slug ] = Object.assign( {}, this.allAvaPlugins[ slug ], pluginData );
			},
			...
			licenseAction: function() {
				...
				self.ajaxLicenseAction = jQuery.ajax( {
					type: 'POST',
					url: window.AvaDashboardPageConfig.ajaxUrl,
					dataType: 'json',
					data: {
						action: 'ava_license_action',
						data: {
			...
			executeAction: function() {
				...
				self.ajaxLicenseAction = jQuery.ajax( {
					...
					dataType: 'json',
					data: {
						action: 'ava_dashboard_debug_action',
						data: {
