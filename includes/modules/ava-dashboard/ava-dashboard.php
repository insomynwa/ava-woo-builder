...
class Dashboard {
	 ...
	 * @var string
	 */
	public $dashboard_slug = 'ava-dashboard';
	...
	public function init( $args = [] ) {
		...
		if ( ! $this->path || ! $this->url || ! $this->args['cx_ui_instance'] ) {
			wp_die(
				'Ava_Dashboard not initialized. Module URL, Path, UI instance and plugin data should be passed into constructor',
				'Ava_Dashboard Error'
			);
		}
	...
	public function register_page() {
		...
		// add_submenu_page(
		// 	'ava-dashboard',
		// 	esc_html__( 'Dashboard', 'ava-tricks' ),
		// 	esc_html__( 'Dashboard', 'ava-tricks' ),
		// 	'manage_options',
		// 	'ava-dashboard'
		// );
	...
	public function enqueue_assets( $hook ) {
		 ...
		 */
		do_action( 'ava-dashboard/before-enqueue-assets', $this );

		/**
		 * Fires before enqueue page assets with dynamic subpage name
		 */
		do_action( 'ava-dashboard/before-enqueue-assets/' . $this->get_subpage(), $this );

		$direction_suffix = is_rtl() ? '-rtl' : '';

		wp_enqueue_style(
			'ava-dashboard-admin-css',
			$this->url . 'assets/css/ava-dashboard-admin' . $direction_suffix . '.css',
			false,
			$this->version
		);

		wp_enqueue_script(
			'ava-dashboard-script',
			$this->url . 'assets/js/ava-dashboard.js',
			array( 'cx-vue-ui' ),
			$this->version,
			true
		);
		...
		wp_localize_script(
			'ava-dashboard-script',
			'AvaDashboardPageConfig',
			apply_filters( 'ava-dashboard/js-page-config', array(
				...
				'allAvaPlugins'     => $this->plugin_manager->get_plugin_data_list(),
	...
	public function print_vue_templates() {
		...
		$templates = apply_filters(
			'ava-dashboard/js-page-templates',
			array(
		...
		foreach ( $templates as $name => $path ) {
			...
			printf(
				'<script type="text/x-template" id="ava-dashboard-%1$s">%2$s</script>',
				$name,
				$content
			);
		}
	...
	public function get_view( $path ) {
		return apply_filters( 'ava-dashboard/get-view', $this->path . 'views/' . $path . '.php' );
	}
